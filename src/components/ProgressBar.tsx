...
import { downloadZip } from '../lib/exports';
...
export function ProgressBar() {
  ...
  const bytes = `${formatBytes(flow.originalTotal)} → ${formatBytes(flow.outputTotal)}`;
  const counts =
    flow.errors > 0
      ? `${flow.done} of ${flow.total} · ${bytes}`
      : `${flow.done} ${flow.done === 1 ? 'image' : 'images'}${autoDownload ? ' saved' : ''} · ${bytes}`;
  ...
  const settingsLabel =
    formats.length === 1
      ? `${FORMAT_META[formats[0][0]].label} · ${formatDetail(formats[0][0])}`
      : formats.map(([f, n]) => `${FORMAT_META[f].label} ×${n}`).join(' · ');
